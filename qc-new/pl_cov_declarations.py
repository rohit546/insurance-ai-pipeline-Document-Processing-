...
5. **EVIDENCE REQUIREMENTS:**
   ...
   - **SPECIAL RULE FOR BUSINESS INCOME "ALS":** For Businessowners Policies (BOP/BP 00 03):
     * If you find "Business Income" or "Business Income and Extra Expense" coverage INCLUDED without a dollar limit → this IS ALS
     * If you find "Extended Business Income: XX Days" → this CONFIRMS Business Income coverage exists (on ALS basis)
     * Evidence can be: "Business Income: Included", "Business Income and Extra Expense: Included", "Extended Business Income: 60 Consecutive Days Included"
     * These all indicate ALS coverage in a BOP - return MATCH with the evidence found
...
**CRITICAL - BUSINESSOWNERS POLICY (BOP) ALS RECOGNITION:**
- In Businessowners Policies (BP 00 03 forms), Business Income is ALWAYS written on an "Actual Loss Sustained" basis BY DEFAULT
- The standard BOP form language states: "We will pay for the actual loss of Business Income you sustain..."
- If you see ANY of these indicators, Business Income IS on ALS basis:
  1. "actual loss of Business Income" (form language)
  2. "Actual Loss Sustained" or "A.L.S." or "ALS" explicitly stated
  3. Business Income coverage listed/included WITHOUT a specific dollar limit (in BOPs, this means ALS)
  4. "Business Income and Extra Expense" coverage included (standard BOP coverage is ALS)
  5. "Extended Business Income: XX Consecutive Days" - this is an EXTENSION that CONFIRMS Business Income is covered (on ALS basis)

- If certificate value is "Actual Loss Sustained" (or similar like "A.L.S." or "ALS"):
  - MATCH if ANY of the above ALS indicators are found
  - MATCH if policy shows Business Income coverage is INCLUDED without a stated dollar limit (this IS ALS in BOPs)
  - MATCH if "Extended Business Income" extension is shown (confirms BI coverage exists on ALS basis)
  - MISMATCH ONLY if policy shows a specific dollar limit that conflicts with ALS
...
- If the policy lists Business Income as part of a combined "Business Income and Extra Expense" or similar, this IS the ALS coverage - treat as MATCH.
- "Extended Business Income: 60 Consecutive Days" or similar extensions CONFIRM that Business Income coverage exists (on ALS basis).
- Waiting period (e.g., 72 hours) supports inclusion but is NOT the BI limit; capture it as context.

- Do NOT confuse Business Income with:
  - Extra Expense only (without Business Income)
